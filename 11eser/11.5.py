...
def main():
	...
	for _ in range(N_toys) :
		...
		p_value = 1- chi2.cdf(Q_squared, df = ndof)
	...
	plt.figure(figsize=(10, 6))
	N_bins = sturges(len(p_value_results))
	bin_edges = np.linspace(np.min(p_value_results), np.max(p_value_results), N_bins)
	plt.hist(p_value_results, bin_edges, label = f'Distribuzione del p_value con {ndof} gradi di libertà', alpha=0.5, density = True)
	plt.legend()
	plt.xlabel("x")
	plt.ylabel("y")
	plt.show()
